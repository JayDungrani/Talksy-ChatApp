...
export const sendFriendReq = createAsyncThunk("request/send", async (receiverId, { rejectWithValue }) => {
    try {
        const { data } = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/friend/send/${receiverId}`, { withCredintials:true });
        return data
    } catch (error) {
...
export const acceptFriendReq = createAsyncThunk("request/accept", async (reqId, { rejectWithValue }) => {
    try {
        const {data} = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/friend/accept/${reqId}`, { withCredintials:true });
        return data;
    } catch (error) {
        return rejectWithValue(error.response?.data || "Something went wrong");
    }
...
export const rejectFriendReq = createAsyncThunk("request/reject", async (reqId, { rejectWithValue }) => {
    try {
        const {data} = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/friend/reject/${reqId}`, { withCredintials:true });
        return data;
    } catch (error) {
        return rejectWithValue(error.response?.data || "Something went wrong");
    }
